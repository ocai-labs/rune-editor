...
describe("MediaImport inside a column", () => {
  // A media block on a nested column surface. The import layer resolves it by
  // id (start / apply), prunes by id (collectLiveMediaIds), and scans by id
  // (collectPendingHtmlImports) — all of which were root-only before the
  // body-surface migration. Left column holds the media; right column a
  // paragraph so the 2-column minimum is met.
  function columnVideoDoc() {
    return {
      type: "doc",
      content: [
        {
  ...
  it("startMediaUrlImport resolves onto a media block nested in a column", async () => {
    const pending = deferred<RuneMediaImportResult>()
    const importMediaUrl = vi.fn(() => pending.promise)
    const editor = createTestEditor({
      kit: { importMediaUrl },
      content: columnVideoDoc(),
    })
    ...
    expect(importMediaUrl).toHaveBeenCalledWith("https://source.example/clip.mp4", {
      blockId: "colVideo",
      kind: "video",
      nodeName: "video",
      source: "embed",
    ...
    expect(getMediaImportState(editor, "colVideo")).toBeUndefined()
    expect(nodeAttrsById(editor, "colVideo")).toMatchObject({
      src: "https://cdn.example/clip.mp4",
      sourceUrl: "https://source.example/clip.mp4",
      title: "Clip",
      width: 1280,
  ...
  it("keeps in-column import state across an unrelated doc change", () => {
    const editor = createTestEditor({ content: columnVideoDoc() })

    // Seed importing state directly so this probes collectLiveMediaIds in
    // isolation (the start command is covered separately above).
    ...
    expect(getMediaImportState(editor, "colVideo")?.phase).toBe("importing")

    // A docChanged elsewhere must NOT prune the in-column media's import state:
    // the live-id scan has to see blocks inside columns, not just root siblings.
    editor.view.dispatch(
  ...
  it("imports a pendingFromPaste image nested in a column", async () => {
    const pending = deferred<RuneImageImportResult>()
    const importImageUrl = vi.fn(() => pending.promise)
    const editor = createTestEditor({
      kit: { importImageUrl },
    ...
    // The import plugin's appendTransaction does not run on the initial content
    // load; a subsequent docChanged elsewhere triggers the pending scan.
    editor.view.dispatch(
      editor.state.tr.insert(
        editor.state.doc.content.size,
        editor.state.schema.nodes.paragraph!.create(),
    ...
    expect(getMediaImportState(editor, "colImg")?.phase).toBe("importing")
    expect(nodeAttrsById(editor, "colImg")?.pendingFromPaste).toBeNull()

    pending.resolve({ src: "https://cdn.example/a.png", width: 100, height: 80, alt: "A" })
    await flushPromises()

    expect(nodeAttrsById(editor, "colImg")).toMatchObject({
